...
irl = irl.sort_values(["Year"], ascending=True)
usa = gdp_eu[(gdp_eu["Country Code"]) == "USA"]
usa = usa.sort_values(["Year"], ascending=True)
...
'# create a dataset to plot IRL and USA as two different subplots'
fig, ax=plt.subplots()
ax.plot(irl["Year"], irl["Per Capita"],   marker="v")
ax.plot(usa["Year"], usa["Per Capita"], marker="v")
ax.set(title="Ireland v USA GDP per Capita 2010 - 2016", xlabel="Year", ylabel="Per Capita ('000's)")
fig.tight_layout()
